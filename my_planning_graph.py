...
class PlanningGraph():
    ...
    graph can be used to reason about the given problem.
    """
    ...
    def noop_actions(self, literal_list):
        """
        Creates persistent action for each possible fluent
    ...
    def create_graph(self):
        ...
        The S0 is the initial level, it has no parents and includes all of
    ...
    def add_action_level(self, level):
        ...
        """
        # Added action A level to the planning graph which does the following :-
        # 1. determines what actions to add and create those PgNode_a objects
        # 2. connects the nodes to the previous S literal level
        # for example, the A0 level will iterate through all possible actions for the problem and add a PgNode_a to a_levels[0]
        #   set iff all prerequisite literals for the action hold in S0.  This is accomplished by testing
        ...
        for action in self.all_actions:
            ...
            if set(a_node.prenodes).issubset(self.s_levels[level]):
                a_nodes.add(a_node)
    ...
    def add_literal_level(self, level):
        ...
        """
        # Added literal S level to the planning graph which does the following :-
        # 1. determines what literals to add
        # 2. connects the nodes
    ...
    def update_a_mutex(self, nodeset):
        """ 
        Determines and updates sibling mutual exclusion for A-level nodes
    ...
    def serialize_actions(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        """
        Tests a pair of actions for mutual exclusion, returning True if the
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        """
        Tests a pair of actions for inconsistent effects, returning True if
        one action negates an effect of the other, and False otherwise.

        The Action instance associated with an action node is accessible
        ...
        :return: bool
        """
        # Testing for Inconsistent Effects between nodes
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # Testing for Interference between nodes

        for eff_add in node_a1.action.effect_add:
            if eff_add in node_a2.action.precond_neg:
                return True

        for eff_add in node_a1.action.effect_rem:
            if eff_add in node_a2.action.precond_pos:
                return True
        ...
        for eff_add in node_a2.action.effect_rem:
            if eff_add in node_a1.action.precond_pos:
                return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Testing for Competing Needs between nodes
    ...
    def update_s_mutex(self, nodeset: set):
        """ 
        Determines and updates sibling mutual exclusion for S-level nodes
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        """
        Tests a pair of state literals for mutual exclusion, returning True if
        one node is the negation of the other, and False otherwise.

        Look at the PgNode_s.__eq__ defines the notion of equivalence for
        ...
        :return: bool
        """
        # Testing for negation between nodes

        if (node_s1.symbol == node_s2.symbol) and (not node_s1.is_pos or not node_s2.is_pos):
            return True
        return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        The PgNode.is_mutex method can be used to test whether two nodes
        ...
        :return: bool
        """
        # Testing for Inconsistent Support between nodes
    ...
    def h_levelsum(self) -> int:
        """
        The sum of the level costs of the individual goals (admissible if goals are independent)
        ...
        # for each goal in the problem, determines the level cost, then adds them together

        for goal in self.problem.goal:
            goal_found = False
            level = 0
            while (not goal_found) and (level < len(self.s_levels)):
                for state in self.s_levels[level]:
                    literal = expr(state.symbol)
                    if not state.is_pos:
                        literal = expr('~{}'.format(state.symbol))
